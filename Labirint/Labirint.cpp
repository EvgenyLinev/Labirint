...
bool dfs_to_key(int x, int y)// deep-first scan
{
    mazem[yt - 1][xt - 1] = '*';
    ...
    {
        *point = '.';
        if (dfs_to_key(x, y + 1)) { *point = '.'; return true; }
        if (dfs_to_key(x + 1, y)) { *point = '.'; return true; }
        if (dfs_to_key(x, y - 1)) { *point = '.'; return true; }
        if (dfs_to_key(x - 1, y)) { *point = '.'; return true; }
        //return true;
    }
...
bool dfs_from_key(int x, int y)// deep-first scan
{
    mazem[yt - 1][xt - 1] = '*';
    char* point = &mazem[y][x];
    if (*point == 'F') { return true; }

    if (*point == ' ')
    {
        *point = '.';
        if (dfs_from_key(x, y + 1)) { *point = '.'; return true; }
        if (dfs_from_key(x + 1, y)) { *point = '.'; return true; }
        if (dfs_from_key(x, y - 1)) { *point = '.'; return true; }
        if (dfs_from_key(x - 1, y)) { *point = '.'; return true; }
        //return true;
    }
...
int main()
{
    ...
    *str = 'S';
    ...
    if (!dfs_to_key(x, y)) { cout << "No path" << endl; }
    if (!dfs_from_key(xt - 1, yt - 1)) { cout << "No path" << endl; }
